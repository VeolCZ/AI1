from w2.CSP_solver import add_to_dict
from w2.Variable import Variable
from w2.utils import connect_variables_and_constraints
...
class CSP:
    ...
    def __init__(
        ...
        if heuristic is not None and heuristic != "mrv" and heuristic != "deg":
            print(
                f"Unknown heuristic '{heuristic}'. Poosible arguments are: None, 'mrv', 'deg'."
            )
            exit(1)
    ...
    def print_statistic(self):
        print(
            "Number Solutions: {0}; Number States visited {1}; Number constraints evaluated {2}".format(
                self.n_solutions_found, self.n_states, self.n_constraints_evaluated
            )
        )
    ...
    def arc_consistency(
        self, binary_constraints, variables_to_check=None
    ) -> tuple[bool, dict]:
    ...
    def solve_rec(self, verbose):
        ...
        if verbose:
            self.print_with_indent(
                f"Choosing variable {variable.name} with domain {variable.domain}"
            )
        ...
        for d in variable.domain:
            ...
            if self.keep_node:
                ...
                conflict, node_domain_reductions = self.node_consistency(
                    new_unary_constraints
                )
                if verbose:
            ...
            if self.keep_arc and not conflict:
                new_binary_constraints = variable.constraints_with_degree(
                    n_unassigned=2
                )
                conflict, arc_domain_reductions = self.arc_consistency(
                    new_binary_constraints, list(node_domain_reductions.keys())
                )

                if verbose:
                    for x in arc_domain_reductions.keys():
                        self.print_with_indent(
                            f"arc: remove {arc_domain_reductions[x]} from the domain of {x.name}"
                        )
                    if conflict:
    ...
    def solve(self, verbose=False):
        ...
        if self.init_node:
            ...
            if verbose:
                for x in node_domain_reductions.keys():
                    self.print_with_indent(
                        f"init_node: remove {node_domain_reductions[x]} from the domain of {x.name}"
                    )
            if conflict:
                if verbose:
                    print(
                        "empty domain found while making problem initially node consistent!"
                    )
                return
        ...
        if self.init_arc:
            ...
            if verbose:
                for x in arc_domain_reductions.keys():
                    self.print_with_indent(
                        f"init_arc: remove {arc_domain_reductions[x]} from the domain of {x.name}"
                    )

            if conflict:
                if verbose:
                    print(
                        "empty domain found while making the problem initially arc consistent!"
                    )
                return
